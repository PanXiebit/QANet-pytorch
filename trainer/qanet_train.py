...
from trainer.metrics import convert_tokens, evaluate_by_dict
...
class Trainer(object):
    ...
    def __init__(self, args, model, loss,
                 train_data_loader, dev_data_loader, logger,
                 optimizer, scheduler, ema,
                 identifier='',):
        self.device = torch.device("cuda" if args.with_cuda else "cpu")
        self.args = args
        ...
        self.dev_eval_dict = pickle_load_large_file(args.dev_eval_file)
        self.is_debug = args.debug
        self.debug_batchnum = args.debug_batchnum
        self.logger = logger
        self.unused = True  # whether scheduler has been updated

        self.lr = args.lr
        self.lr_warm_up_num = args.lr_warm_up_num
            ...
            if self.use_early_stop:
                if result["f1"] < self.best_f1 and result["em"] < self.best_em:
                    ...
                    if patience > self.early_stop:
                        self.logger.info("Perform early stop!")
                        break
                else:
                    patience = 0
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.train_data_loader):
            ...
            if self.step % self.print_freq == self.print_freq - 1:
                ...
                self.logger.info(("step: {}/{}, epoch: {}, lr: {:.4f}, loss: {:.4f}, "
                       "speed: {:.4f} examples/sec").format(
        ...
        self.logger.info("dev_em: %f \t dev_f1: %f" % (
